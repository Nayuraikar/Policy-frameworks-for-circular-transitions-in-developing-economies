...
set.seed(42)
fit_path_age <- sem(path_model, data = df, group = "age_split", se = "bootstrap", bootstrap = 1000)

ests <- parameterEstimates(fit_path_age) %>%
  filter(op == "~") %>%
...
levels <- lavInspect(fit_path_age, "group.label")
cat("\nGroup 1:", levels[1], "(N =", sum(df$age_split == levels[1], na.rm=TRUE), ") |  Group 2:", levels[2], "(N =", sum(df$age_split == levels[2], na.rm=TRUE), ")\n\n")
